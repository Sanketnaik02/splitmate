      ...
      {members.map((member) => (
        <div key={member.userId} className="flex items-center gap-3 px-4 py-2.5 group">
          ...
          <div className="flex-1 min-w-0">
            ...
            {member.role === 'admin' && <span className="text-[10px] text-gray-500 dark:text-gray-300 font-medium">Admin</span>}
          </div>
          <span className={`text-sm font-semibold ${(member.balance || 0) > 0 ? 'text-green-600' : (member.balance || 0) < 0 ? 'text-red-600' : 'text-gray-500 dark:text-gray-300'}`}>
