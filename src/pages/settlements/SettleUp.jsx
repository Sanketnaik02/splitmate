  ...
  if (!activeGroup) {
    return (
      <AppLayout userName={user?.displayName || 'User'}>
        <div className="text-center py-16"><p className="text-4xl mb-3">🔍</p><p className="text-gray-500">Group not found</p></div>
      </AppLayout>
    );
  }
    ...
    <AppLayout userName={user?.displayName || 'User'}>
      <div className="pt-1">
        <div className="flex items-center gap-3 mb-4">
          <button onClick={() => navigate(`/groups/${groupId}`)} className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-gray-600">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">{activeGroup.name}</p>
          </div>
        </div>

        <Card padding="p-5" className="mb-4 text-center">
          <p className="text-sm text-gray-500 mb-1">Your balance in this group</p>
        ...
        {suggestedPayments.length === 0 ? (
          ...
          <div className="space-y-3">
            ...
            {suggestedPayments.length > 0 && (
              <Card padding="p-4">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">All suggested payments</p>
                <div className="space-y-2">
